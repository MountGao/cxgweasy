...
class special_act extends act
{
    ...
    function show_action()
    {
        ...
        $limit = (($this->view->page - 1) * $this->pagesize) . ',' . $this->pagesize;
        $special = new special();
        $this->view->special = $special->getrow('spid=' . intval(front::get('spid')));
        $this->view->archive['title'] = $this->view->special['title'];
        $this->view->pages = true;
        $archive = new archive();
        $archives = $archive->getrows('spid=' . intval(front::get('spid')), $limit);
        ...
        $this->view->record_count = $archive->rec_count('spid=' . intval(front::get('spid')));
        front::$record_count = $this->view->record_count;
